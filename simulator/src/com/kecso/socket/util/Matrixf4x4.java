 ...
 *
 * [ x0 , y0 , z0 , w0 ] [ x1 , y1 , z1 , w1 ] [ x2 , y2 , z2 , w2 ] [ x3 , y3 ,
 * z3 , w3 ]
 *
 * it is recommend that when setting the matrix values individually that you use
 * the set{x,#} methods, where 'x' is either x, y, z or w and # is either 0, 1,
 * 2 or 3, setY1 for example. The reason you should use these functions is
 * because it will map directly to that part of the matrix regardless of whether
 * or not the internal matrix is column major or not. If the matrix is either or
 * length 9 or 16 it will be able to determine if it can set the value or not.
 * If the matrix is of size 9 but you set say w2, the value will not be set and
 * the set method will return without any error.
 *
 */
public class Matrixf4x4 {
     ...
     * however you can change this by using the setColumnMajor function to false
     * and it will operate like a row major matrix.
     */
    public Matrixf4x4() {
    ...
    /**
     * Sets the matrix from a float[16] array. If the matrix you set isn't 16
     * long then the matrix will be set as invalid.
     *
    ...
    /**
     * Set whether the internal data is col major by passing true, or false for
     * a row major matrix. The matrix is column major by default.
     *
     * @param colMajor
     */
    ...
    /**
     * Multiply the given vector by this matrix. This should only be used if the
     * matrix is of size 16 (use the matrix.size() method).
     *
    ...
    /**
     * Multiply the given vector by this matrix. This should only be used if the
     * matrix is of size 9 (use the matrix.size() method).
     *
    ...
    public void multiplyMatrix4x4ByMatrix(Matrixf4x4 matrixf) {
        ...
        if (matrixValid && matrixf.isMatrixValid()) {
             ...
             *
             * int k = i * 4; bufferMatrix[0 + j] += this.matrix[k + j] *
             * matrix[0 * 4 + i]; bufferMatrix[1 * 4 + j] += this.matrix[k + j]
             * * matrix[1 * 4 + i]; bufferMatrix[2 * 4 + j] += this.matrix[k +
             * j] * matrix[2 * 4 + i]; bufferMatrix[3 * 4 + j] += this.matrix[k
             * + j] * matrix[3 * 4 + i]; } }
             */
    ...
    /**
     * This will rearrange the internal structure of the matrix. Be careful
     * though as this is an expensive operation.
     */
